...
# snow_survey_fixed <- snow_survey_fixed %>% 
#   rowwise() %>% 
#   mutate(Total_cover_recalc = sum(c(Snow_cover, Water_cover, Land_cover), 
#                                   na.rm = TRUE)) %>% 
#   ungroup() %>% 
#   mutate(Total_cover_recalc = ifelse(c(Snow_cover, Water_cover, Land_cover) == is.na(), NA, Total_cover_recalc))
...
There are almost 4,600 rows that fit the criteria above. This is not good as that is about 10% of the entire dataframe. We could potentially change these values to 0 or 100 as they represent the minimum and maximum percentage, but I am going to keep it as is.
...
# write the file
write_csv(snow_survery_fixed, file.path(datadir_processed, "all_cover_fixed_Tom_Gibbens_Matsuyama.csv"))
