...
Now that these three columsn are cleaned, let's recompute the `Total_cover`. Since, we only have numeric percentages, when we recompute we should not have any non-numeric values besides `NA`.
...
# Check to see if total cover has only NA non-numeric values
snow_survey_fixed %>% 
